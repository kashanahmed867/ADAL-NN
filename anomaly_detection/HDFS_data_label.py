...
faults = 'ijlog.csv'
raw_filename = 'HDFS_2k.log'
structured_filename = 'HDFS_2k.log_structured.csv'
template_filename = 'HDFS_2k.log_templates.csv'
anomaly_tags = dict()

def customShuffle(block_logs, block_ti, train_len):
    # train_idices = random.sample(range(0, len(block_logs)), train_len)
    train_idices = random.sample(range(0, len(block_logs)), len(block_logs))
...
for blk_id, is_anomaly in anomaly_tags.items():
    if is_anomaly:
        if blk_id in block_logs.keys():
            abnormal_block_logs.append(block_logs[blk_id])
        if blk_id in block_ti.keys():
